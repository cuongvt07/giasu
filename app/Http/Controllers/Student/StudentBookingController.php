...
class StudentBookingController extends Controller
{
    ...
    public function create(Tutor $tutor)
    {
        ...
        // Lấy quản lý lịch rảnh của gia sư (cả lịch cụ thể và lịch lặp lại)
        $availabilities = $tutor->availabilities()
            ->where(function ($query) {
                $query->where('start_time', '>', now()) // quản lý lịch rảnh cụ thể trong tương lai
    ...
    public function store(Request $request, Tutor $tutor)
    {
        ...
        // Log để kiểm tra
        \Illuminate\Support\Facades\Log::info('Tính toán Kết nối gia sư hỗ trợ nhanh', [
        ...
        return redirect()->route('payment.create', $booking)
            ->with('success', 'Đã tạo Kết nối gia sư hỗ trợ nhanh thành công, vui lòng hoàn tất thanh toán');
    }
